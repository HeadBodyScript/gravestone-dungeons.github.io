...
execute as @s[scores={tick=15}] run summon husk ~ ~-2.7 ~ {CustomName:'[{"text":"Poisoning Sword Guardian","color":"#336600"}]',Health:70,PersistenceRequired:1b,Tags:["companion"],Team:companion,active_effects:[{id:invisibility,duration:-1,amplifier:0,ambient:1b,show_particles:1b}],HandItems:[{id:stone_sword,components:{enchantments:{levels:{knockback:1,"gd_enchantment:poisoning":1,sharpness:1,smite:1,sweeping_edge:3,unbreaking:10}}},count:1},{id:shield,components:{enchantments:{levels:{unbreaking:10}},banner_patterns:[{pattern:rhombus,color:white},{pattern:flower,color:brown},{pattern:stripe_middle,color:black},{pattern:globe,color:lime},{pattern:piglin,color:green},{pattern:circle,color:black},{pattern:triangle_top,color:black},{pattern:curly_border,color:black}],base_color:black},count:1}],HandDropChances:[0.25f,0f],ArmorItems:[{id:chainmail_boots,components:{enchantments:{levels:{frost_walker:1,thorns:3,unbreaking:10}}},count:1},{id:chainmail_leggings,components:{enchantments:{levels:{thorns:3,unbreaking:10}}},count:1},{id:chainmail_chestplate,components:{enchantments:{levels:{thorns:3,unbreaking:10}}},count:1},{id:chainmail_helmet,components:{enchantments:{levels:{thorns:3,unbreaking:10}}},count:1}],ArmorDropChances:[0f,0f,0f,0f],attributes:[{id:"generic.max_health",base:70f},{id:"minecraft:generic.knockback_resistance",base:90}]}

execute as @s[scores={tick=2}] run playsound minecraft:entity.slime.attack master @s ~ ~ ~ 1 .5
execute as @s[scores={tick=2}] run playsound minecraft:block.fire.extinguish master @s ~ ~ ~ 1 1.8
