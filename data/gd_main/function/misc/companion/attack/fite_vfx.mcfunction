...
execute as @a[scores={check=1}] at @p if entity @e[type=#gd_main:companion_can_attack,distance=..20] run scoreboard players add @p fireblock_dummy 1
...
execute as @a[scores={fireblock_time=1}] at @e[type=#gd:companion,tag=fireguard,limit=1,sort=random] run summon armor_stand ~ ~ ~ {Tags:["fireblock"],Invisible:1b,Passengers:[{id:"minecraft:item_display",Tags:["fireblock"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1.2f,1.2f,1.2f]},item:{id:"minecraft:magma_block"}}]}
...
execute as @a[scores={fireblock_time=2}] at @e[type=armor_stand,tag=fireblock] run tp @e[type=armor_stand,tag=fireblock] ^ ^ ^ facing entity @e[type=#gd_main:companion_can_attack,tag=!companion,sort=nearest,limit=1] feet
...
#execute as @a[scores={check=1}] as @e[type=#gd_main:companion_can_attack,distance=..20] run say fire