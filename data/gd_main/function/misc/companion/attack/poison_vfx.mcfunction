...
execute as @a[scores={check=1}] at @p if entity @e[type=#gd_main:companion_can_attack,distance=..20] run scoreboard players add @p poisonblock_dummy 1
...
execute as @a[scores={poisonblock_time=2}] at @e[type=armor_stand,tag=poisonblock] run tp @e[type=armor_stand,tag=poisonblock] ^ ^ ^ facing entity @e[type=#gd_main:companion_can_attack,tag=!companion,sort=nearest,limit=1] feet
...
#execute as @a[scores={check=1}] as @e[type=#gd_main:companion_can_attack,distance=..20] run say earth
