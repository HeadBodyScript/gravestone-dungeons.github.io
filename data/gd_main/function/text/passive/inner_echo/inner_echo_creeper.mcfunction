...
execute if score .INT0 randomNumber matches 1 run tellraw @s ["",{"text":"Caelen Voren","color":"blue"},{"text":": *whispers* Do not panic… but a green fiend hides nearby. I know it."}]
# execute if score .INT0 randomNumber matches 1 run playsound minecraft:inner_echo_trigger_1 master @s ~ ~ ~ 10 1

execute if score .INT0 randomNumber matches 2 run tellraw @s ["",{"text":"Caelen Voren","color":"blue"},{"text":": There lies a beast nearby... foul, green, and steeped in wickedness. Keep thy wits sharp."}]
# execute if score .INT0 randomNumber matches 2 run playsound minecraft:inner_echo_trigger_2 master @s ~ ~ ~ 10 1

execute if score .INT0 randomNumber matches 3 run tellraw @s ["",{"text":"Caelen Voren","color":"blue"},{"text":": Something slithers close... a creeper, if my senses still serve."}]
# execute if score .INT0 randomNumber matches 3 run playsound minecraft:inner_echo_trigger_3 master @s ~ ~ ~ 10 1

execute if score .INT0 randomNumber matches 4 run tellraw @s ["",{"text":"Caelen Voren","color":"blue"},{"text":": Hm… I feel a presence. A creeper, perhaps. Vile beast."}]
# execute if score .INT0 randomNumber matches 4 run playsound minecraft:inner_echo_trigger_4 master @s ~ ~ ~ 10 1

execute if score .INT0 randomNumber matches 5 run tellraw @s ["",{"text":"Caelen Voren","color":"blue"},{"text":": *whispers* Cast thine eyes about... A creeper skulks close."}]
