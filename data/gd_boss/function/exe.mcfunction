...
execute if score INT difficulty matches 1.. as @e[type=minecraft:marker,tag=marker.boss,tag=!FALSE] at @s if entity @a[distance=..50,gamemode=!spectator] run function gd_boss:init
...
# marker traps
execute if score INT difficulty matches 1.. as @e[type=minecraft:armor_stand,tag=armor_stand.trap,limit=1,sort=random] at @s if entity @a[distance=..8,gamemode=!spectator] run function gd_boss:trap/init
