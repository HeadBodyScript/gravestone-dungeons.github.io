...
execute if score @s bossTick matches 1 unless score @s boss.grimgar_minion_INT <= grimgar.minion.count INT as @e[type=minecraft:marker,distance=..32,tag=marker.minion,limit=2,sort=random] at @s run function gd_boss:grimgar/other/minion
