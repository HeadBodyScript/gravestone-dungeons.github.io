...
execute as @e[type=#gd_main:entity,tag=!companion,distance=..1.5] unless score @s enchantment.knife_pouch_UUID = @e[type=minecraft:armor_stand,limit=1,distance=..1.5] enchantment.knife_pouch_UUID run function gd_enchantment:knife_pouch/other/hit with storage minecraft:damage
