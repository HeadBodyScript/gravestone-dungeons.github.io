...
execute as @a[scores={check=1}] at @p if entity @e[type=#gd:mobs,distance=..20] run scoreboard players add @p iceblock_dummy 1
...
execute as @a[scores={iceblock_time=2}] at @e[type=armor_stand,tag=iceblock] run tp @e[type=armor_stand,tag=iceblock] ^ ^ ^ facing entity @e[type=#gd:mobs,tag=!companion,sort=nearest,limit=1] feet
...
#execute as @a[scores={check=1}] as @e[type=#gd:mobs,distance=..20] run say frost