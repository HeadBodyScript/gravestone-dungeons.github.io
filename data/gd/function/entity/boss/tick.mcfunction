...
# GRIMGAR NEW
execute as @e[type=minecraft:armor_stand,tag=spawngrimgarbox,tag=boss_marker] at @s if entity @a[distance=..10] run function gd:entity/boss/grimgar/init
execute as @e[type=minecraft:armor_stand,tag=spawngrimgarbox,tag=boss_marker,tag=TRUE] at @s if score @s playerCount matches 1.. run function gd:entity/boss/grimgar/tick

execute at @e[type=minecraft:armor_stand,tag=spawngrimgarbox,tag=boss_marker,tag=!TRUE] run particle dust_color_transition{from_color: [.0f, .0f, .0f], scale: 1f, to_color: [.1f, .1f, .1f]} ~ ~3 ~ .2 1 .2 0 30 force
