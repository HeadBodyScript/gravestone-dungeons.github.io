...
execute as @e[scores={an=50, randomnumber=..9}] at @e[type=minecraft:armor_stand,tag=an_spe] run title @p[distance=..6] actionbar [{"text":"You walk under the sign of agility.","color":"white"}]
execute as @e[scores={an=50, randomnumber=..9}] at @p run particle minecraft:effect ~ ~1 ~ 0.5 1 0.5 0.05 20 force
execute as @e[scores={an=50, randomnumber=..9}] at @e[type=minecraft:armor_stand,tag=an_spe] run effect give @p[distance=..6] minecraft:speed 1200 1 true

execute as @e[scores={an=50, randomnumber=10}] at @e[type=minecraft:armor_stand,tag=an_spe] run title @p[distance=..6] actionbar [{"text":"The wrath of the gods be upon thee, for thy actions displease them.","color":"gray"}]
execute as @e[scores={an=50, randomnumber=10}] at @e[type=minecraft:armor_stand,tag=an_spe] run effect clear @p[distance=..6] minecraft:speed
execute as @e[scores={an=50, randomnumber=10}] at @e[type=minecraft:armor_stand,tag=an_spe] run effect give @p[distance=..6] minecraft:slowness 300 2 true