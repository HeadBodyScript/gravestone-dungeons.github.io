...
execute as @e[scores={an=50, randomnumber_1=..9}] at @e[type=minecraft:armor_stand,tag=an_mag] run title @p[distance=..6] actionbar [{"text":"You walk under the sign of faster magicka regeneration for 20 minutes.","color":"blue"}]
execute as @e[scores={an=50, randomnumber_1=..9}] at @p run particle minecraft:end_rod ~ ~ ~ .5 2 .5 0.001 50 force
execute as @e[scores={an=50, randomnumber_1=..9}] at @e[type=minecraft:armor_stand,tag=an_mag] run effect give @p[distance=..6] minecraft:wind_charged 1200 0 true
execute as @e[scores={an=50, randomnumber_1=..9}] at @e[type=minecraft:armor_stand,tag=an_mag] run scoreboard players set @p magicka_boost 1
execute as @e[scores={an=50, randomnumber_1=..9}] at @e[type=minecraft:armor_stand,tag=an_mag] run scoreboard players set @p magicka_boost_time 600

execute as @e[scores={an=50, randomnumber_1=10}] at @e[type=minecraft:armor_stand,tag=an_mag] run title @p[distance=..6] actionbar [{"text":"The wrath of the gods be upon thee, for thy actions displease them.","color":"gray"}]

execute as @e[scores={an=50, randomnumber_1=10}] at @e[type=minecraft:armor_stand,tag=an_mag] run scoreboard players set @p magicka_boost 0
execute as @e[scores={an=50, randomnumber_1=10}] at @e[type=minecraft:armor_stand,tag=an_mag] run effect clear @p minecraft:wind_charged
execute as @e[scores={an=50, randomnumber_1=10}] at @e[type=minecraft:armor_stand,tag=an_mag] run effect give @p[distance=..6] minecraft:infested 120 0 true
