##Frosty Sword Guardian##
scoreboard objectives add conjure dummy
scoreboard players add @p conjure 1

execute at @e[type=armor_stand,tag=conjure_frosty_sword] run tp @e[type=armor_stand,tag=conjure_frosty_sword] ^ ^0.20 ^ ~12 ~
...
execute as @a[scores={conjure=25..}] run kill @e[type=armor_stand,tag=conjure_frosty_sword,limit=2,sort=nearest]
